...
<!--FORM-->  
    <?php
    $vac_info = $conn->query("SELECT id, company, vacancy, city, search, web, web_link, politely, professionally, interested, operatively, conformity, communication, salary, other1, other2, other3, status, manager, date FROM vacs WHERE status='archive'");
    while ($row_vac_info = $vac_info->fetch_assoc()) {
    echo "<center><div class='blok8'><a name='".$row_vac_info["id"]."'></a>";
        echo "".$row_vac_info["id"].",&ensp;".$row_vac_info["manager"].",&ensp;".$row_vac_info["date"]."";
        echo "<form method='post' action='zy_exe.php'>
            ...
            <input type='submit' class='form_btn1' name='enter_archive_admin' value='Зберегти'>
        </form><br>
    </div></center>";}
    ?>
